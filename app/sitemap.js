import { PORTFOLIO, PROJECT, SUB_MENU } from "./data/data";

export default function sitemap() {
  const services = SUB_MENU.map(({ link }) => {
    return {
      url: `${process.env.NEXT_PUBLIC_BASE_URL}/Services/${link}`,
      lastModified: new Date(),
      priority: 0.5,
    };
  }); // 0.5 is the default priority
  const projects = PORTFOLIO.map(({ siteMaplink }) => {
    return {
      url: `${process.env.NEXT_PUBLIC_BASE_URL}/Projects${siteMaplink}`,
      lastModified: new Date(),
      priority: 0.5, // 0.5 is the default priority
    };
  });
  const Portfolio = PROJECT.map(({ link }) => {
    return {
  ...
  return [
    ...
    {
      url: "https://cdc.construction/ContactUs",
      lastModified: new Date(),
      changeFrequency: "weekly",
      priority: 0.5,
    },
    {
      url: "https://cdc.construction/CallBack",
      lastModified: new Date(),
      changeFrequency: "weekly",
      priority: 0.5,
    },
